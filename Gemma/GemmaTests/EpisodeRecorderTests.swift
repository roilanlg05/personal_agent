...
final class EpisodeRecorderTests: XCTestCase {
    func test_record_writes_user_and_assistant_conversation_nodes() throws {
        ...
        let convos = try store.allNodes().filter { $0.kind == NodeKind.conversation.rawValue }
    ...
    func test_episodes_do_not_count_as_facts() throws {
        ...
        XCTAssertTrue(try store.allNodes().filter { $0.kind != NodeKind.conversation.rawValue }.isEmpty)
    }
}
